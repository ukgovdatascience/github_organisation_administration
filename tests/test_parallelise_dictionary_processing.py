...
# Define test cases for the `TestParalleliseProcessing` test class
args_test_parallelise_processing_callable_function = [lambda x: {x: x ** 2}, lambda x: x ** 2]
args_test_parallelise_processing_iterable = [[1, 2, 3]]
args_test_parallelise_processing_cpu_count = [1, 3, 5, 7]
args_test_parallelise_processing_max_chunksize = [1, 4, 8]


@pytest.mark.parametrize("test_input_callable_function", args_test_parallelise_processing_callable_function)
@pytest.mark.parametrize("test_input_iterable", args_test_parallelise_processing_iterable)
@pytest.mark.parametrize("test_input_cpu_count", args_test_parallelise_processing_cpu_count)
@pytest.mark.parametrize("test_input_max_chunksize", args_test_parallelise_processing_max_chunksize)
class TestParalleliseProcessing:

    def test_pool_called_once_correctly(self, patch_multiprocessing_pool: MagicMock,
                                        test_input_callable_function: Callable, test_input_iterable: iter,
                                        test_input_cpu_count: int, test_input_max_chunksize: int) -> None:
        """Test `multiprocessing.Pool` is called once correctly."""

        # Execute the `parallelise_processing` function
        _ = parallelise_processing(test_input_callable_function, test_input_iterable, test_input_cpu_count,
    ...
    def test_returns_correctly(self, patch_multiprocessing_pool_enter_imap_unordered: MagicMock,
        ...
        # Execute the `parallelise_processing` function
        test_output = parallelise_processing(test_input_callable_function, test_input_iterable, test_input_cpu_count,
                                             test_input_max_chunksize)
...
class TestParalleliseDictionaryProcessing:
    ...
    def test_returns_correctly(self, patch_parallelise_processing: MagicMock,
                               test_input_callable_function: Callable[..., Dict], test_input_iterable: iter,
                               test_input_cpu_count: int, test_input_max_chunksize: int) -> None:
        """Test the output of the function is as expected."""
        ...
        # Assert the output is as expected
        assert test_output == {k: v for d in patch_parallelise_processing.return_value for k, v in d.items()}
